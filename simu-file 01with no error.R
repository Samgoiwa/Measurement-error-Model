...
#sigma_u_L =0.5#0.05 0.10 0.15 0.25 0.50
#sigma_u_S =0.5#0.05 0.10 0.15 0.25 0.50 
x<-cbind(1, land_size, soil_characteristics)

# true beta coefficients
true_beta_coef<-c(100, 50,5)

# true sigma for Optimal fertilizer
true_sigma <- 50
I<-diag(1,n,n) # n by n identity covariance matrix 

# simulate Optimal fertilizer in tons  
...
#error covariates
x<-cbind(1, land_size + rnorm(n,0,sigma_u_L), soil_characteristics+rnorm(n,0,sigma_u_S))
...
bayes_res<-lapply(y_list, blocked_gibbs, x=x, iter=1000, burnin=50, thin=1)
